...
/* HARTES */
bool pragma_call_hw_found = false;
tree function_call_found = NULL;
bool function_decl_found = false;
tree save_pragma_stmt;
extern tree call_hw_tree;  
...
c_lex_one_token (c_parser *parser, c_token *token)
{
  ...
  switch (token->type)
	...
	parser->objc_need_raw_identifier = false;
      ...
      {
        if (pragma_call_hw_found && !function_call_found && !function_decl_found)
          {
            tree call = get_call_expr_in (save_pragma_stmt);
            int fpga_id = 0;
            if (call)
              fpga_id = TREE_INT_CST_LOW (CALL_EXPR_ARG(call, 0));
            printf ("pragma %d not on a function decl/call\n", fpga_id);
          }
	/*if(pragma_call_hw_found && function_call_found){
	  debug_tree(function_call_found);
	  }*/
        pragma_call_hw_found = false;
        function_call_found = NULL;
        function_decl_found = false;
      }
					   ...
					   struct c_expr expr)
{
  ...
  while (true)
	...
	case CPP_OPEN_PAREN:
	  ...
	  /* HARTES */
	  function_call_found = expr.value;
	  break;
	case CPP_DOT:
...
c_parser_pragma (c_parser *parser, enum pragma_context context)
{
  ...
  if (call_hw_tree) 
    {
      tree stmt = built_in_decls[BUILT_IN_MOLEN_DUMMY];
      tree decl;
      tree node = TREE_VALUE(call_hw_tree);
      //tree node = call_hw_tree;
      bool first = true;
      int cnt=0;
	...
	debug_tree(nodedeb);
	nodedeb = TREE_CHAIN(nodedeb);
      }
      */
      stmt = build_call_expr(stmt, 1, node);
      decl = get_callee_fndecl (stmt);
      add_stmt (stmt);
      pragma_call_hw_found = true;
      save_pragma_stmt = stmt;
      if(cfun){
	/* if we are in the body func let's reset*/
